...
def getOpenAiMsg(name, question):
    ...
    if info[name]['上下文联系数量'] <= 1:  # 如果上下文数量设置为1
        info[name]['messages'] = [{'role': 'user', 'content': question}] 
    else:
        info[name]['messages'].append({'role': 'user', 'content': question})  
...
def 群聊用户回复():
    ...
    if we.Exists(0):
        ...
        we.Click(simulateMove=False)
        last_msg = wx.ListControl(Name="消息").GetChildren()[-1].Name  # 获取最新消息
        last_msg = last_msg[7:len(last_msg)]  
        # print(wx.ListControl(Name="消息").GetChildren()[-1]) 
        # print(last_msg == '权限')
